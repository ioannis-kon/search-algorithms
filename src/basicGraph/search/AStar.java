...
public class AStar implements SearchAlgorithms {
    ...
    /**
     * Function that initializes A* search
    ...
    public void traverse(Graph g, String start, String goal) {
        ...
        System.out.println("====================");
        System.out.println("A* Search Traversing...");
        System.out.println("====================");
    ...
    private void traverse(int v, String goal, boolean[] visited, Graph g) {
        ...
        priorityQueue = new PriorityQueue(g.getVertices().size(), new AStarHeuristics());
        //mark current vertex as visited
        visited[v] = true;
        System.out.println("Evaluating : " + g.getVertices().get(v).getName() + " with heuristic value : f(n) = g(n) + h(n) = " + g.getVertices().get(v).getCost());
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            for (Neighbor nbr = g.getVertices().get(evaluationVertex).adjList; nbr != null; nbr = nbr.next) {
                if(!visited[nbr.vertexNum] && evaluationVertex != nbr.vertexNum) {
                    //add to current vertex the cost from previous path up to this point
                    g.getVertices().get(nbr.vertexNum).addToCostSoFar(g.getVertices().get(evaluationVertex).getCost() + g.getVertices().get(evaluationVertex).getCostSoFar());
                    System.out.println(g.getVertices().get(evaluationVertex).getName() + " --> " + g.getVertices().get(nbr.vertexNum).getName());
                    System.out.println("Evaluating : " + g.getVertices().get(nbr.vertexNum).getName() + " with heuristic value : f(n) = g(n) + h(n) = " + g.getVertices().get(nbr.vertexNum).getTotalCost());
    ...
    private int getVertexWithMinimumHeuristicValue(Graph g) {
        ...
        for(int i=0;i<g.getVertices().size();i++) {
            if(v.getName().equals(g.getVertices().get(i).getName())) return i;
        }
        return -1;
    }
